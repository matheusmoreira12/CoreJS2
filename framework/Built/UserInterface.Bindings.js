...
export const IBindingOptions = new Interface(new InterfaceProperty("direction", Type.get(Number), MemberSelectionAttributes.Any, true), new InterfaceProperty("valueConverter", IValueConverter, MemberSelectionAttributes.Any, true));
...
class PropertyBindingWorker extends BindingWorker {
    ...
    updateTargetProperty(value) {
        ...
        if (!Enumeration.isFlagSet(BindingDirection.ToTarget, options.direction))
            return;
    ...
    updateSourceProperty(value) {
        const options = this.getComputedOptions();
        if (!Enumeration.isFlagSet(BindingDirection.ToSource, options.direction))
            return;
        const valueConverter = options.valueConverter;
        if (valueConverter !== null)
