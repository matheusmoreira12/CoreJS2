...
function applyWorkerMethods(self, worker, ...methods) {
    ...
    function applyGetterAndSetter(getter, setter) {
        const hasGetter = !!getter, hasSetter = !!setter;
        const name = (getter || setter).name, workerGetterName = WORKER_GETTER_PREFIX + name, workerSetterName = WORKER_SETTER_PREFIX + name;
        const attributes = {};
        if (hasGetter)
    ...
    while (reader.isWithinBounds) {
        ...
        reader.increment();
        if (appliedNames.includes(method.name))
            continue;
        switch (method.type) {
