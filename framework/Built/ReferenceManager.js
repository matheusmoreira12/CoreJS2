    ...
    (function (Standard) {
        ...
        class ReferenceRetrievalRequest extends Promise {
            constructor(name, context) {
                this.status = ReferenceRetrievalRequestStatus.Pending;
                let _resolve;
                super(function (resolve, reject) {
                    _resolve = resolve;
                });
                if (typeof name !== "string")
                    throw new ArgumentTypeException("name", name, String);
                if (!(context instanceof ReferenceManagerContext))
                    throw new ArgumentTypeException("context", context, ReferenceManagerContext);
                this.resolve = (value) => {
                    _resolve(value);
        ...
        class Declaration {
            constructor(name, context, value) {
                if (typeof name !== "string")
                    throw new ArgumentTypeException("name", name, String);
                if (!(context instanceof ReferenceManagerContext))
                    throw new ArgumentTypeException("parentContext", parentContext, ReferenceManagerContext);
                this.name = name;
                this.context = context;
                this.value = value;
            }
        }
        class ReferenceManagerContext {
            constructor(target = null, parentContext = null) {
                this._ownDeclarations = new Map();
                if (name && typeof name !== "string")
                    throw new ArgumentTypeException("name", name, String);
                if (parentContext && !(parentContext instanceof ReferenceManagerContext))
                    throw new ArgumentTypeException("parentContext", parentContext, ReferenceManagerContext);
                this.name = name;
        ...
        Standard.ReferenceManagerContext = ReferenceManagerContext;
        ...
        function requestMatchesDeclaration(request, declaration) {
            if (request.name !== declaration.name)
                return false;
            if (request.context !== declaration.context &&
                !request.context.isContainedWithin(declaration.context))
                return false;
            return true;
        }
        function* getMatchingPendingRequests(declaration) {
            for (let request of requests) {
                if (request.status !== ReferenceRetrievalRequestStatus.Pending)
                    continue;
                if (!requestMatchesDeclaration(request, declaration))
                    continue;
                yield request;
            }
        }
